...
class QR8bitByte {
    constructor(data) {
        ...
        for (let i = 0; i < this.data.length; i++) {
            ...
            if (code > 65536) {
                bytes[0] = 0xF0 | ((0x1C0000 & code) >>> 18);
                bytes[1] = 0x80 | ((0x3F000 & code) >>> 12);
                bytes[2] = 0x80 | ((0xFC0 & code) >>> 6);
                bytes[3] = 0x80 | (0x3F & code);
            } else if (code > 2048) {
                bytes[0] = 0xE0 | ((0xF000 & code) >>> 12);
                bytes[1] = 0x80 | ((0xFC0 & code) >>> 6);
                bytes[2] = 0x80 | (0x3F & code);
            } else if (code > 128) {
                bytes[0] = 0xC0 | ((0x7C0 & code) >>> 6);
                bytes[1] = 0x80 | (0x3F & code);
            } else {
                bytes[0] = code;
            }
            
            this.parsedData = this.parsedData.concat(bytes);
        }
    ...
    write(bitBuffer) {
        for (let i = 0; i < this.parsedData.length; i++) {
            bitBuffer.put(this.parsedData[i], 8);
        }
    }
}
...
class QRRSBlock {
    ...
    static getRsBlockTable(typeNumber, errorCorrectLevel) {
        switch (errorCorrectLevel) {
            case ErrorCorrectLevel.L: return QRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 0];
            case ErrorCorrectLevel.M: return QRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 1];
            case ErrorCorrectLevel.Q: return QRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 2];
            case ErrorCorrectLevel.H: return QRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 3];
...
function getTypeNumber(data, errorCorrectLevel) {
    const length = getUTF8Length(data);
    ...
    for (let typeNumber = 1; typeNumber <= 40; typeNumber++) {
        ...
        const lengthInBits = QRUtil.getLengthInBits(MODE_8BIT_BYTE, typeNumber);
        if (length * 8 <= totalDataCount * 8 - 4 - lengthInBits) {
            return typeNumber;
        }
    }
...
class QRDrawing {
    ...
    draw(qrCode) {
        ...
        const ratio = Math.min(
            this._htOption.width / moduleCount,
            this._htOption.height / moduleCount
        );
...
export class QRCode {
    constructor(element, options) {
        ...
        this._el = element;
        this._oQRCode = null;
        this._oDrawing = new QRDrawing(this._el, this._htOption);
    ...
    makeCode(text) {
        this._oQRCode = new QRCodeModel(
            getTypeNumber(text, this._htOption.correctLevel),
...
QRCode.CorrectLevel = ErrorCorrectLevel;
